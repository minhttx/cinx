...
*   **Ollama - Local LLM Runtime:** 
    ...
    - Model hỗ trợ: Gemini 3 Flash, là mô hình LLM do Google phát triển, chạy qua proxy của Ollama.
...
*   **Bộ não xử lý (Multi-stage NLU & Orchestration):** CinX không gửi trực tiếp câu hỏi của người dùng tới LLM chính để tránh tình trạng AI "nói hươu nói vượn" (Hallucination). Thay vào đó, hệ thống vận hành một quy trình **Orchestration** 2 giai đoạn:
    ...
    -  **Prompt Engineering chuyên sâu:** Chúng ta sử dụng kỹ thuật **Few-shot Prompting** và **Chain-of-Thought (CoT)** ẩn để hướng dẫn AI trích xuất thực thể. AI được cung cấp các ví dụ mẫu về cách chuyển đổi ngôn ngữ đời thường sang dữ liệu máy tính, giúp độ chính xác của NLU đạt trên 95% ngay cả với các mô hình ngôn ngữ nhỏ chạy local.
*   **Cơ chế Phản hồi & Hiển thị (Interactive Streaming UI):** Giao diện chatbot của CinX được thiết kế để mang lại cảm giác phản hồi tức thì và tương tác hai chiều sâu sắc:
    ...
    -  **Markdown & GFM Rendering:** Sử dụng `react-markdown` kết hợp với `remark-gfm` để hỗ trợ hiển thị các cấu trúc dữ liệu phức tạp như Bảng (Tables) và Danh sách (Lists). AI được chỉ dẫn luôn trình bày lịch chiếu dưới dạng bảng Markdown để khách hàng dễ dàng so sánh giữa các khung giờ và loại phòng.
    -  **Thiết kế phân tầng (Z-Index Layering):** Để đảm bảo chatbot luôn là ưu tiên tương tác số 1, cửa sổ chat được cấu hình `z-index: 3000` kết hợp với một lớp `nav-global-overlay` (`z-index: 2999`). Lớp phủ này sử dụng `backdrop-filter: blur(4px)` để làm mờ hậu cảnh, giúp người dùng tập trung hoàn toàn vào nội dung tư vấn của AI, đồng thời ngăn chặn các thao tác click nhầm vào các thành phần UI phía dưới.
    -  **Chỉ dẫn hành động (CTA Engineering):** AI được lập trình thông qua System Prompt để luôn kết thúc câu trả lời bằng một lời kêu gọi hành động (Call to Action) kèm theo hyperlink, ví dụ: *"Bạn có muốn đặt vé suất [19:00](showtime:...) ngay không?"*. Chiến thuật này làm tăng tỷ lệ chuyển đổi đặt vé thông qua chatbot một cách đáng kể.
...
### 3.2.1. **Hệ thống AI Writing (Sáng tạo nội dung tự động):** 
...
### 3.2.2. **Tự động hóa thông tin phim (TMDB API Integration):** 
...
### 3.2.3.  **Quản lý Prompt Hệ thống (Dynamic Agent Configuration):** 
...
### 3.2.4. Hệ thống kiểm duyệt bình luận bằng AI (AI Sentiment Moderation)
Đây là một ứng dụng NLP (Natural Language Processing) thực tế giúp tối ưu hóa quy trình quản lý cộng đồng và làm giàu dữ liệu cho cơ chế RAG.

* **Cơ chế Phân tích cảm xúc (Sentiment Analysis Pipeline):**
    - **Tiền xử lý (Sanitization):** Nội dung bình luận được làm sạch và chuẩn hóa định dạng trước khi gửi tới LLM để đảm bảo độ chính xác của ngữ cảnh.
    - **Định hướng bối cảnh (Contextual Prompting):** AI được cung cấp vai trò "Chuyên gia kiểm duyệt CinX" kèm bối cảnh cụ thể về trải nghiệm rạp phim (phim, ghế, âm thanh, dịch vụ). Điều này giúp AI phân biệt được các sắc thái ngôn ngữ đặc thù của ngành điện ảnh.
    - **Định dạng đầu ra chuẩn hóa:** AI trả về kết quả dưới dạng JSON gồm: `score` (0-1), `label` (Positive/Negative/Neutral) và `reason` (Lý do phân loại).
* **Tối ưu hóa Hiệu suất & Độ bền bỉ:**
    - **Cơ chế Cache nội bộ:** Sử dụng `sentimentCache` (Map) để lưu trữ các kết quả phân tích trong phiên làm việc, giảm 40-60% số lượng lời gọi tới LLM cho các nội dung trùng lặp hoặc khi Admin chuyển đổi qua lại giữa các tab lọc.
    - **Lưu trữ bền vững (Persistence):** Kết quả phân tích được đồng bộ trực tiếp vào database Supabase (`ai_sentiment_score`, `ai_sentiment_label`, `ai_sentiment_reason`). Khi tải trang, hệ thống ưu tiên sử dụng dữ liệu đã lưu, giúp giao diện phản hồi tức thì mà không cần quét lại.
    - **Xử lý lỗi thông minh:** Tích hợp các khối logic Fallback và Validation chặt chẽ. Nếu AI trả về định dạng sai hoặc kết nối bị ngắt, hệ thống tự động gán nhãn `Neutral` và điểm số `0.5` để không làm gián đoạn quy trình của Admin.
* **Giá trị vận hành (Business Logic):**
    - **AI Scan All:** Cho phép quét hàng loạt bình luận đang chờ duyệt chỉ bằng một cú click.
    - **Duyệt nhanh AI (Smart Bulk Approve):** Thuật toán tự động lọc các bình luận có điểm tích cực > 70% để Admin có thể phê duyệt hàng loạt, giúp tiết kiệm 90% thời gian kiểm duyệt thủ công.
    - **Social Proof Loop:** Các bình luận tích cực sau khi được phê duyệt sẽ ngay lập tức được cơ chế RAG thu thập để chatbot CinX sử dụng làm dẫn chứng khi tư vấn cho người dùng khác, tạo ra một vòng lặp uy tín tự động cho hệ thống.
...
---
**Ngày cập nhật báo cáo:** 25/03/2026
